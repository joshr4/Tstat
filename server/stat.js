...
var stat = {
    ...
    unoccSch: {},
    temp: {
        raw: 0, value: 70, res: 10, celsius: 20,
        A: 0.002425308852122,
        B: 0.000419711863267,
        C: 0.000000595663516
    },  //temp
...
stat.updateCh = () => {
    adc.ch0() //get temp
        .then(data => {
            ...
            stat.temp.celsius = 1 / (stat.temp.A + stat.temp.B * Math.log(stat.temp.res) + stat.temp.C * Math.pow(Math.log(stat.temp.res), 3)) - 273.15
            stat.temp.value = stat.temp.celsius * 1.8 + 32
            adc.ch1() //get dial
                .then(data => {
                    stat.dial.raw = data;
